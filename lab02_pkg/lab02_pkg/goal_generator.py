...
class GoalGeneratorActionClient(Node):
    def __init__(self):
        super().__init__("goal_generator_action_client", namespace="/turtle1")
        ...
        self.move_distance_client = ActionClient(self, MoveDistance, "move_distance")
    ...
    def rotate_absolute_get_feedback(self, feedback_msg):
        ...
        self.get_logger().info(f"Remaining: {degrees(feedback.remaining)} degrees", throttle_duration_sec=1.0)

    def move_distance_get_feedback(self, feedback_msg):
        feedback = feedback_msg.feedback
        self.get_logger().info(f" Remaining: {feedback.remaining_distance} meters", throttle_duration_sec=1.0)
...
async def run(args, loop):
    ...
    ################################################################################################
    for i in range(2):   
        goal = [random()*10, random()*10, random()*6.28]
